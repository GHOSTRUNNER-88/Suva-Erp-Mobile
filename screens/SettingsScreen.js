...
export default function SettingsScreen() {
  ...
  const fullName = [user?.firstName, user?.lastName].filter(Boolean).join(" ") || t("profile.notProvided", "Not provided");

  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <ScreenHeader title={t("common.settings", { defaultValue: "Settings" })} />
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <Animated.View style={enterAnim}>
            ...
            <View style={styles.heroCard}>
              ...
              <Text style={styles.email}>{user?.email || t("profile.notProvided", "Not provided")}</Text>
              {user?.role ? (
            ...
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Feather name="user-check" size={16} color={colors.primary} style={styles.cardHeaderIcon} />
                <Text style={styles.sectionTitle}>{t("profile.accountDetails", { defaultValue: "Account Details" })}</Text>
              </View>
              <InfoRow icon="briefcase" label={t("profile.role", { defaultValue: "Role" })} value={user?.role || t("profile.notProvided", "Not provided")} />
              <InfoRow icon="home" label={t("profile.organization", { defaultValue: "Organization" })} value={data?.organization?.name || t("profile.notProvided", "Not provided")} />
